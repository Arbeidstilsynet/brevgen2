...
export function createInMemoryGitProvider(seed: InMemorySeed): GitProvider {
  ...
  function readFileContent(repoId: string, branch: string, filePath: string): Promise<string> {
  ...
  return {
    ...
    async readManyFileContents(repoId, branch, filePaths) {
      ...
      for (const filePath of filePaths) {
        results.push({ filePath, content: await readFileContent(repoId, branch, filePath) });
      }
      return results;
    },
  };
